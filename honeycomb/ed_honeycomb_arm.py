...
def nn(r,Lx,Ly):
    ...
    if r % 4 == 0:
        if i != Lx:
            return r+1, ((i+1)*Ly+j)*2+1, (i*Ly+(j-1)%Ly)*2+1
        else:
        ...
        if i == 0:
            ...
            return r-1, r+1, ((i-1)*Ly+j)*2
    elif r % 4 == 2:
        if i == 0:
            return r-1, r+1
        else:
            return r-1, r+1, ((i-1)*Ly+j)*2+1
    else:
        if i != Lx:
            return r-1, (i*Ly+(j+1)%Ly)*2, ((i+1)*Ly+j)*2
        else:
...
plt.savefig(f"plot/E0_honeycomb_{Lx}_{Ly}_arm.pdf",format='pdf')
