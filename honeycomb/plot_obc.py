...
tc_dic = {(4,2):[-0.25,-2],(4,4):[-0.35,-0.65],(4,8):[-0.3,-0.9],
          (4,12):[-0.05,-0.65],(4,27):[0.05,-2],
          (6,2):[-0.25,-2],(6,4):[-0.25,-0.65],(6,8):[-0.35,-0.75],
          (6,12):[-0.25,-0.9],(6,16):[-0.25,-0.55],
          (6,20):[0.05,-0.65],(6,36):[-0.05,-2],(6,39):[0.05,-2]}

E_F = np.fromfile(f"data/E_2l_obc_N_{Nx}_n_{m}_p_F_chi_{chi}.dat")
E_A = np.fromfile(f"data/E_2l_obc_N_{Nx}_n_{m}_p_A_chi_{chi}.dat")
#E_A2 = np.fromfile(f"data/E_ex_N_{Nx}_n_{m}_p_A_chi_{chi}.dat")
S0_F = np.fromfile(f"data/S_2l_obc_N_{Nx}_n_{m}_p_F_chi_{chi}.dat")
S0_A = np.fromfile(f"data/S_2l_obc_N_{Nx}_n_{m}_p_A_chi_{chi}.dat")
...
plt.xlabel("$t_1$")
plt.ylabel("$E_0$")
plt.savefig(f"plot/E0_2l_obc_Nx_{Nx}_n_{m}_chi_{chi}.pdf",format='pdf')
...
plt.xlabel("$t_1$")
plt.ylabel("$S^{tot}$")
plt.savefig(f"plot/S0_2l_obc_Nx_{Nx}_n_{m}_chi_{chi}.pdf",format='pdf')
plt.show()
