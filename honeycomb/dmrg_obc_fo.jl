...
function main(Nx::Int, Ny::Int, t::Float64, t1::Float64, p::String, m::Int,
    ...
    f1 = h5open("data/data0/H_2l_obc_$(Nx)_n_$(m)_p_$(p)_t1_$(t1)_chi_$(chi0).h5","r")
    H = read(f1,"H",MPO)
    close(f1)

    f2 = h5open("data/data0/psi_2l_obc_$(Nx)_n_$(m)_p_$(p)_t1_$(t1)_chi_$(chi0).h5","r")
    psi0 = read(f2,"psi",MPS)
    close(f2)

    energy,psi = dmrg(H,psi0; nsweeps, maxdim, cutoff) #, noise)

    f1 = h5open("data/data0/H_2l_obc_$(Nx)_n_$(m)_p_$(p)_t1_$(t1)_chi_$(chi).h5","w")
    write(f1,"H",H)
    close(f1)

    f2 = h5open("data/data0/psi_2l_obc_$(Nx)_n_$(m)_p_$(p)_t1_$(t1)_chi_$(chi).h5","w")
    write(f2,"psi",psi)
    close(f2)
...
for p in ["F", "A"]
    ...
    open("data/E_2l_obc_N_$(Nx)_n_$(m)_p_$(p)_chi_$(chi).dat","w") do io
        write(io, E_l)
    end
    open("data/S_2l_obc_N_$(Nx)_n_$(m)_p_$(p)_chi_$(chi).dat","w") do io
        write(io, S_l)
    end 
end
