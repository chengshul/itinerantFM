...
function main(Nx::Int, t::Float64, t1::Float64, J::Float64, J1::Float64,
    ...
    f1 = h5open("data/data0/H_2l_$(Nx)_n_$(m)_p_$(p)_t1_$(t1)_chi_1000.h5","w")
    write(f1,"H",H)
    close(f1)

    f2 = h5open("data/data0/psi_2l_$(Nx)_n_$(m)_p_$(p)_t1_$(t1)_chi_1000.h5","w")
    write(f2,"psi",psi)
    close(f2)
...
for p in ["F", "A"]
    ...
    open("data/E_2l_N_$(Nx)_n_$(m)_p_$(p)_chi_1000.dat","w") do io
        write(io, E_l)
    end
    open("data/S_2l_N_$(Nx)_n_$(m)_p_$(p)_chi_1000.dat","w") do io
        write(io, S_l)
    end
end
